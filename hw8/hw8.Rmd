...
contagion_sim <- function(p, network, nodes, steps) {
  ...
  for (step in 2:steps) {
    ...
    # might not have included i. Therefore, we have to remove the j doctors
...
generate_abc_sample = function(observed_data,
    ...
    #     if(sum(abs(summary_statistic(y) -  summary_statistic(observed_data))) < epsilon) {
    #         return(theta)
    #     }
    theta = prior_distribution()
    y = data_generating_function(theta)
    y[is.na(y)] = 99
    # resid = sum(abs(summary_statistic(y) -  summary_statistic(observed_data)))
    resid = sum(abs(y - observed_data))
    return(data.frame(theta, resid))
}
```
...
data_generating_function = function(x) {contagion_sim(p = x, network = network,
                                                      nodes = initially_infected_nodes, 
                                                      steps = 18)                               #max(attributes$adoption_date)
  }
observed_data = attributes$adoption_date
summary_statistic = function()
epsilon = .05

posterior_samples = replicate(n = 10,
